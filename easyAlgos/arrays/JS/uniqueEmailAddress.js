...
*/
// emails is an array of strings/emails
const numUniqueEmails = (emails) => {
  for (let i = 0, set = new Set(), email; i < emails.length; ++i) {
    email = emails[i].split('@');
    email[0] = email[0].split('+')[0].replace(/\./g, '');
    set.add(email.join('@'));
    if (i === emails.length - 1) return set.size;
  }
};
...
console.log(numUniqueEmails(emails1)); // 2
console.log(numUniqueEmails(emails2)); // 3
